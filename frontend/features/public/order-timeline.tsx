...
/**
 * How long they have been waiting, in the words somebody actually uses.
 *
 * Recomputed on a timer rather than read during render, because calling the clock
 * while rendering is impure and a figure that only moved when something else happened
 * would sit frozen through the longest part of the wait - which is exactly the part
 * this is for.
 *
 * Every half minute. A counter ticking every second turns a wait into a stopwatch,
 * and watching a stopwatch makes ten minutes feel like twenty.
 */
function useElapsed(placedAtUtc: string): string | null {
  ...
  const placed = new Date(placedAtUtc).getTime();

  if (Number.isNaN(placed)) {
    return null;
  }

  const minutes = Math.max(0, Math.round((now - placed) / 60_000));
